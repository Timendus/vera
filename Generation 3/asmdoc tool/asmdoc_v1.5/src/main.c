...
{
	printf("\nASMDOC tool v1.5 - Reference implementation for Vera coding standards\n");
	...
	if (argc == 3 || argc == 4) {
		...
		if ( is_dir(argv[1]) ) {
			...
			// Start building the routines XML file
			char routinesfile[ BUFSIZ ];
			char *routinesfileptr = routinesfile;
			strcpy(routinesfileptr,argv[2]);
			routinesfileptr += strlen(argv[2]);
#ifdef WINDOWS
			strcpy(routinesfileptr,"\\routines.xml");
#else
			strcpy(routinesfileptr,"/routines.xml");
#endif
			routines = fopen(routinesfile,"w");
			if ( tree == NULL ) {
				perror(routinesfile);
				printf("\n");
				return -1;
			}
			fprintf(routines,"<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>\n");
			fprintf(routines,"<?xml-stylesheet type=\"text/xsl\" href=\"veradoc.xsl\"?>\n\n");
			fprintf(routines,"<routines>\n");
			
			// Parse files in directory
			parse_dir(argv[1],argv[2],layout);
			
			// Close routines XML file
			fprintf(routines,"</routines>\n");
			fclose(routines);
...
void register_routine( char *name ) {
	if ( pass == 2 ) {
		if ( routineOpen ) close_routine();
		if ( routines != NULL )
			fprintf(routines,"\t<routine>\n\t\t<name>%s</name>\n\t\t<file>%s</file>\n\t</routine>\n",name,rel_to_dest_dir(currentFile));
